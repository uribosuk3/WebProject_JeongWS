...
public class QnaBoardModifyServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
        ...
        req.setAttribute("pageTitle", "Q&A 게시글 수정");
        req.setAttribute("board", board);
        // 💡 JSP 경로 수정 반영
        req.getRequestDispatcher("/WEB-INF/views/qna/modify.jsp").forward(req, resp);
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
        ...
        // 4. 결과 처리
        if (result) {
            resp.sendRedirect(req.getContextPath() + "/qna/view.do?idx=" + idx);
        } else {
            resp.setContentType("text/html; charset=UTF-8");
